...
class App:
    ...
    def __init__(self, propotion,video_src, roiwid, graph_path, roihei,type,is_selected, paused = False):
    ...
    def process_frame(self, gFrame, t0, scale):
        ...
        if self.init_flow:
            opt_flow = cv2.calcOpticalFlowFarneback(prev_frame, gFrame, None,    0.5, 5, 13, 10, 5, 1.1,cv2.OPTFLOW_FARNEBACK_GAUSSIAN)
            self.init_flow = False
        else:
            if(np.sum(self.opt_flow) is not None):
                opt_flow = cv2.resize(self.opt_flow, (0,0), None, scale, scale)
                opt_flow = cv2.calcOpticalFlowFarneback(prev_frame, gFrame, opt_flow, 0.5, 5, 13, 10, 5, 1.1,cv2.OPTFLOW_USE_INITIAL_FLOW)
            else:
                opt_flow = cv2.calcOpticalFlowFarneback(prev_frame, gFrame, None,    0.5, 5, 13, 10, 5, 1.1,cv2.OPTFLOW_FARNEBACK_GAUSSIAN)
                self.init_flow = False
    ...
    def IsExistPoint(self, point, croped_pos):
        if(point[0][::-1] > (croped_pos[0],croped_pos[1])):
            if(point[0][::-1] < (croped_pos[2],croped_pos[3])) : 
                if(point[1][::-1] > (croped_pos[0],croped_pos[1])):
                    if(point[1][::-1] < (croped_pos[2],croped_pos[3])) : 
                        return True
        return False
    #-------------------------------------------------------------------------!>
    ...
    def run(self):
        ...
        while True:
            ...
            while len(self.pending) > 0 and self.pending[0].ready():
                ...
                if self.isFindbloodLine == False:
                    ...
                    # cv2.waitKey(0)
                    # self.GetROI(eroded)
                    closed = cv2.morphologyEx(eroded, cv2.MORPH_CLOSE,cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)),iterations=3)
                    ...
                    if self.isCanDetectCellsCount == False : 
                        ...
                        text = 'Average Speed : {:.2f} mm / s, Cells Count : {:.0f}'.format(self.averageSpeed/self.ratio, self.averageCount)
                        ...
                        draw_str(print_result, (20, 480), "frame interval :  %.1f ms" % (self.frame_interval.value*1000))
                        x1, y1, x2, y2  = self.croped_pos
                        draw_str(print_result, (20, 500), "ROI width :  %dmm, height : %dmm" % ((x2-x1)*(25.4 / 96), (y2-y1)*(25.4 / 96)))
            ...
            if len(self.pending) < self.threadn:
                ...
                if(self.is_croped == False):
                    ...
                    if self.is_selected:
                        ...
                        cv2.putText(frame, "Press SPACE key for next step!",(20, 40), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 0, 255), 1); 
                    else:
                        cv2.putText(frame, "Select ROI By MouseDroping",(20, 40), cv2.FONT_HERSHEY_TRIPLEX, 0.7, (0, 0, 255), 1); 

                    cv2.imshow('Select ROI', frame)

                    ch = cv2.waitKey(1)
                    
                    if self.manual==-2 or self.is_selected:
                    ...
                    process_img = self.process_img = cv2.resize(self.base_frame, (0,0), None, self.rePropotion, self.rePropotion)
        ...
        draw_str(blank_image, (100 + point,  40), "----- Result -----")

        text = 'Average Speed : {:.6f} mm /s'.format(avarageSpeed)
        draw_str(blank_image, (point, 80), text)

        text = 'Average Count : {:.0f}'.format(avarageCount)
        draw_str(blank_image, (point,  100), text)
...
# print(videos_path)
root.wm_title("Chose Options")
root.geometry("250x300") 
  
b1 = tk.Button(root,text = "Select ROI Automatic",command=lambda: selFunc(True),bg='#567', fg='White',height=4,width=20)  
b2=tk.Button(root,text="Select ROI Manually",command=lambda: selFunc(False),bg='#567', fg='White',height=4,width=20)
